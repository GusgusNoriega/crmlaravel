...
function verificarContenido{{ $idPrincipal }}(contenido) {
    if (contenido.length >= 2 ) {
        ...
        if (!isNaN(contenido) && !isNaN(parseFloat(contenido))) {
            ...
            buscarElementos{{ $idPrincipal }}(contenido, 'title', '{{ route('buscarProductos') }}');
        } else {
            buscarElementos{{ $idPrincipal }}(contenido, 'title', '{{ route('buscarProductos') }}');
        }
    }
        ...
        if (xhr.status === 200) {
           ...
           // console.log(elementos);
            elementos.forEach(function(elemento) {
                ...
                elementoElement.className = 'producto-individual-suma';
                ...
                <button class="eliminar-elemento-buscador" data-id-eliminar="${elemento.id}"><svg viewBox="-1.7 0 20.4 20.4" xmlns="http://www.w3.org/2000/svg" class="cf-icon-svg"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"><path d="M16.417 10.283A7.917 7.917 0 1 1 8.5 2.366a7.916 7.916 0 0 1 7.917 7.917zm-6.804.01 3.032-3.033a.792.792 0 0 0-1.12-1.12L8.494 9.173 5.46 6.14a.792.792 0 0 0-1.12 1.12l3.034 3.033-3.033 3.033a.792.792 0 0 0 1.12 1.119l3.032-3.033 3.033 3.033a.792.792 0 0 0 1.12-1.12z"></path></g></svg></button>
                   ${rutaImagen}
                    <div class="contenido-opciones-buscador">
                        <h3 class="titulo-producto-div">${elemento.title}</h3>
                    </div>
                    <input class="id-producto" type="hidden" value="${elemento.id}" disabled>
                    <div class="product-div-50">
                        <label class="ocultar product-div-50">Moneda</label> 
                        <input class="moneda-actual" type="text" value="${elemento.moneda_relacionada.code}" disabled>
                    </div>
                    <div class="ocultar product-div-50"> 
                        <label>Moneda final cotizacion</label>
                        <input class="moneda-final" type="text" disabled>
                    </div>
                    <div class="ocultar product-div-50"> 
                        <label>Tipo de cambio</label>
                        <input class="tipo-cambio-actual" type="number" step="0.01" value="${elemento.moneda_relacionada.tipo_cambio}">
                    </div>
                    <div class="ocultar product-div-50"> 
                        <label>Tipo de cambio final</label>
                        <input class="tipo-cambio-final" type="number" step="0.01" value="" disabled>
                    </div>
                    <div class="product-div-50">
                        <label class="ocultar product-div-50">Precio actual</label>
                        <input class="precio-actual" type="number" step="0.01" value="${elemento.precio}" disabled>
                    </div>
                    <div class="ocultar product-div-50">
                        <label class="ocultar">Precio final</label>
                        <input class="precio-tipo-de-cambio" type="number" step="0.01" value="0" disabled>
                    </div>
                    <div class="ocultar product-div-50">
                        <label>catidad</label>
                        <input class="cantidad" type="number" name="cantidad" value="">
                    </div>
                    <div class="ocultar product-div-50"> 
                        <label>Precio descuento</label>
                        <input class="precio-final-producto" type="number" step="0.01" value="">
                    </div>
                    <div class="ocultar product-div-50"> 
                        <label>Total suma</label>
                        <input class="total-suma-producto" type="number" step="0.01" value="" disabled>
                    </div>
                    <!-- Añadir más necesario -->
                `;
...
function actualizarTotal() {
    // Obtenemos todos los divs con la clase 'producto-individual-suma' dentro del contenedor
    const productos = document.querySelectorAll('.elementos-seleccionados .producto-individual-suma');
    const tipoCambioCotizacion = document.getElementById('tipo-de-cambio').value; 
    // Primero, obtienes el elemento <select>
    const selectElement = document.getElementById('idmoneda');
    // Luego, obtienes el índice de la opción seleccionada
    const selectedOptionIndex = selectElement.selectedIndex;
    // Ahora, accedes a la opción seleccionada usando el índice
    const selectedOption = selectElement.options[selectedOptionIndex];
    // Finalmente, obtienes el valor del atributo data- que desees
    const dataNameCode = selectedOption.getAttribute('data-name-code');
    // Iteramos sobre cada producto
    let productosJSON = []; 
    let sumaTotalProductos = 0;
    productos.forEach(producto => {
        // Obtenemos el valor de la cantidad y el precio final del producto

        producto.querySelector('.tipo-cambio-final').value = tipoCambioCotizacion;
        producto.querySelector('.moneda-final').value = dataNameCode;
       
        const tipoCambioProductoActual = producto.querySelector('.tipo-cambio-actual').value;
        const tipoCambioFinal = producto.querySelector('.tipo-cambio-final').value;
        const precioActual = producto.querySelector('.precio-actual').value;
        const precioFinalProducto = producto.querySelector('.precio-final-producto').value;
        const cantidad = producto.querySelector('.cantidad').value;
        const monedaActual = producto.querySelector('.moneda-actual').value;
        const idProducto = producto.querySelector('.id-producto').value;
           
        if (tipoCambioProductoActual) {
            totalProducto = convertirPrecio(precioActual, tipoCambioProductoActual, tipoCambioFinal);
            producto.querySelector('.precio-tipo-de-cambio').value = totalProducto.toFixed(2); 
        }
         
        const precioTipoCambio = producto.querySelector('.precio-tipo-de-cambio').value;
        // Calculamos el total
        let total = 0;
        if(precioFinalProducto >= 1) {
          total = cantidad * precioFinalProducto;
        }else {
            total = cantidad * precioTipoCambio;
        }
        // Establecemos el total en el campo 'total-suma-producto'
        producto.querySelector('.total-suma-producto').value = total.toFixed(2); // Asumiendo que quieres dos decimales

        sumaTotalProductos += total;

        productosJSON.push({
            id: idProducto, 
            tipoCambioActual: tipoCambioProductoActual,
            tipoCambioFinal: tipoCambioFinal,
            precioActual: precioActual,
            precioFinal: totalProducto.toFixed(2),
            monedaActual: monedaActual,
            monedaFinal: dataNameCode,
            cantidad: cantidad,
            precioDescuento: precioFinalProducto,
            total: total.toFixed(2)     
        });
    });

    const productosJSONString = JSON.stringify(productosJSON);
    document.querySelector('#total-productos').value = productosJSONString;
    document.getElementById('total-productos-monto').value = sumaTotalProductos.toFixed(2);
    actualizarTotalGeneral()
    console.log(productosJSONString);
}

document.addEventListener('DOMContentLoaded', function() {
    var contenedor = document.querySelector('#elementos-container-buscadorproductos1');
    ...
    if (contenedor) {
        ...
        console.log('El contenedor especificado no existe.');
    }
});

function convertirPrecio(precioOriginal, tipoCambioOriginal, tipoCambioDeseado) {
    // Convertir el precio original a dólares usando el tipo de cambio original
    const precioEnDolares = precioOriginal / tipoCambioOriginal;
    
    // Convertir el precio en dólares a la moneda deseada usando el tipo de cambio deseado
    const precioConvertido = precioEnDolares * tipoCambioDeseado;
