...
function verificarContenido{{ $idPrincipal }}(contenido) {
    if (contenido.length >= 2) {
        ...
        if (!isNaN(contenido) && !isNaN(parseFloat(contenido))) {
            ...
            buscarElementos{{ $idPrincipal }}(contenido, 'name', '{{ route('buscarCategorias') }}', {{ $subcategorias }});
        } else {
            buscarElementos{{ $idPrincipal }}(contenido, 'name', '{{ route('buscarCategorias') }}', {{ $subcategorias }});
        }
    }
...
function buscarElementos{{ $idPrincipal }}(searchTerm, field, rutaBuscar, subCategorias) {
    // Crear un nuevo objeto XMLHttpRequest
    var xhr = new XMLHttpRequest();
    
    // Configurar el método y la URL de la solicitud
    xhr.open('GET', rutaBuscar + '?searchTerm=' + encodeURIComponent(searchTerm) + '&field=' + encodeURIComponent(field) + '&subCategorias=' + encodeURIComponent(subCategorias), true);
