...
<div class="filtros-principales" id="filtros-principales">
    <div>
        ...
        <div class="seccion-filtro">
            <h3>Ruc</h3>
            <input type="text" id="rucSearchTerm" placeholder="Ruc...">
        </div>
        <div class="seccion-filtro">
        ...
        <div class="seccion-filtro">
            <button class="button button-buscar" onclick="cargarEmpresas()">
                Buscar
...
<div class="resultados-generales-elementos" id="resultadosEmpresas"></div>
<div class="container-botones-sisguiente-anterior">
    ...
    <button type="button" class="button" id="boton-cargar-empresas" data-pagina-siguiente="" style="display: none;">
        pagina siguiente
    ...
    <script>
        ...
        function cargarEmpresas(page = 1) {
                const searchTerm = document.getElementById('searchTerm').value;
                const fechaDesde = document.getElementById('fechaDesde').value;
                const fechaHasta = document.getElementById('fechaHasta').value;
                const rucSearch = document.getElementById('rucSearchTerm').value;
                ...
                fetch('/admin/nuestrasempresas/loadMore', { // Cambia esto por la URL real
                    method: 'POST',
                    headers: {
                ...
                .then(data => {        
                    ...
                    const contenedor = document.getElementById('resultadosEmpresas');
                    contenedor.innerHTML = ''; // Limpia los resultados anteriores
                    /*console.log(data);*/
                    data.misempresas.data.forEach(empresa => {
                        // Suponiendo que 'empresa' tiene una propiedad 'name'
                      

                        
                        const rutaLogo = empresa.imagen_logo ? '<div><img src="http://sistema3.test/storage/' + empresa.imagen_logo.ruta + '" alt="Logo de ' + empresa.imagen_logo.name + '"></div>' : '<div><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path d="M448 80c8.8 0 16 7.2 16 16V415.8l-5-6.5-136-176c-4.5-5.9-11.6-9.3-19-9.3s-14.4 3.4-19 9.3L202 340.7l-30.5-42.7C167 291.7 159.8 288 152 288s-15 3.7-19.5 10.1l-80 112L48 416.3l0-.3V96c0-8.8 7.2-16 16-16H448zM64 32C28.7 32 0 60.7 0 96V416c0 35.3 28.7 64 64 64H448c35.3 0 64-28.7 64-64V96c0-35.3-28.7-64-64-64H64zm80 192a48 48 0 1 0 0-96 48 48 0 1 0 0 96z"/></svg></div>';
                        const rutaSello = empresa.imagen_sello ? '<div><img src="http://sistema3.test/storage/' + empresa.imagen_sello.ruta  + '" alt="Sello de ' + empresa.imagen_sello.name + '"></div>' : '<div><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path d="M448 80c8.8 0 16 7.2 16 16V415.8l-5-6.5-136-176c-4.5-5.9-11.6-9.3-19-9.3s-14.4 3.4-19 9.3L202 340.7l-30.5-42.7C167 291.7 159.8 288 152 288s-15 3.7-19.5 10.1l-80 112L48 416.3l0-.3V96c0-8.8 7.2-16 16-16H448zM64 32C28.7 32 0 60.7 0 96V416c0 35.3 28.7 64 64 64H448c35.3 0 64-28.7 64-64V96c0-35.3-28.7-64-64-64H64zm80 192a48 48 0 1 0 0-96 48 48 0 1 0 0 96z"/></svg></div>';

                        contenedor.innerHTML += `<div>
                                                   <div>
                                                        <button class="button-update-misempresa" data-update-misempresa="${empresa.id}">
                                                        ...
                                                        <button class="button-delete-misempresa" data-delete-misempresa="${empresa.id}">
                                                        ...
                                                        <div class="container-seccion-elemento">
                                                            <p>Nombre</p>
                                                            <div><p>${empresa.name}<p></div>
                                                        </div>
                                                        <div class="container-seccion-elemento">
                                                            <p>Alias</p>
                                                            <div><p>${empresa.alias}</p></div>
                                                        </div>
                                                        <div class="container-seccion-elemento">
                                                            <p>Ruc</p>
                                                            <div><p>${empresa.ruc}</p></div>
                                                        </div>
                                                        <div class="container-seccion-elemento">
                                                            <p>Telefono</p>
                                                            <div><p>${empresa.telefono}</p></div>
                                                        </div>
                                                        <div class="container-seccion-elemento">
                                                            <p>Cuenta en dolares</p>
                                                            <div><p>${empresa.cuenta_dolares}</p></div>
                                                        </div>  
                                                        <div class="container-seccion-elemento">
                                                            <p>Cuenta en soles</p>
                                                            <div><p>${empresa.cuenta_soles}</p></div>
                                                        </div> 
                                                        <div class="container-seccion-elemento">
                                                            <p>Cuenta de la nación</p>
                                                            <div><p>${empresa.cuenta_nacion}</p></div>
                                                        </div>                                                           
                                                    </div>
                                                 </div>`; // Agrega cada empresa al contenedor
                    });
                    const botonCargar = document.getElementById('boton-cargar-empresas');
                    if (data.misempresas.current_page < data.misempresas.last_page) {
                        // No es la última página, muestra el botón y prepara para la próxima página
                        botonCargar.style.display = 'flex'; // Muestra el botón
                        botonCargar.setAttribute('data-pagina-siguiente', data.misempresas.current_page + 1); // Establece la próxima página
                        
                        // Asegúrate de que el botón carga la próxima página cuando se hace clic
                        botonCargar.onclick = () => {
                            cargarEmpresas(data.misempresas.current_page + 1);
                        };
                    } else {
                    ...
                    if (data.misempresas.current_page > 1) {
                        ...
                        botonCargarAnterior.setAttribute('data-pagina-anterior', data.misempresas.current_page - 1); // Establece la página anterior
